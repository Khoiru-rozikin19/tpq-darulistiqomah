...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-6 bg-gradient-to-r from-blue-700 to-blue-800 text-white">
            <h3 class="text-lg font-bold heading-font">Edit Transaksi Kas</h3>
            <p class="text-xs text-blue-100 mt-1">Mengubah rincian transaksi kas tercatat.</p>
        </div>
        ...
        <form action="{{ route('kas.update', $kas->id) }}" method="POST" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="jenis" id="jenis" required class="w-full px-3.5 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs text-slate-700 bg-slate-50/50 font-bold">
                        <option value="masuk" {{ old('jenis', $kas->jenis) === 'masuk' ? 'selected' : '' }}>MASUK (Penerimaan / Hibah / Infaq)</option>
                        <option value="keluar" {{ old('jenis', $kas->jenis) === 'keluar' ? 'selected' : '' }}>KELUAR (Pengeluaran / Operasional / Belanja)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="kategori" id="kategori" list="kategori-suggestions" value="{{ old('kategori', $kas->kategori) }}" required placeholder="Contoh: Infaq, Operasional, ATK..." 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $kas->tanggal->format('Y-m-d')) }}" required 
                ...
                <div>
                    ...
                    <input type="number" name="nominal" id="nominal" value="{{ old('nominal', (int) $kas->nominal) }}" required 
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="4" required placeholder="Jelaskan detail tujuan transaksi ini..." 
                    class="w-full px-3.5 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs text-slate-700 bg-slate-50/50">{{ old('keterangan', $kas->keterangan) }}</textarea>
            </div>
